...
namespace MyAgencyVault.BusinessLibrary
{
  ...
  public class FollowupIssue : IEditable<DisplayFollowupIssue>
  {
      ...
      using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
      {
          ...
          List<DisplayFollowupIssue> _FollowupIssue = (from p in Result
                                                       ...
                                                       select new DisplayFollowupIssue
                                                       {
                                                           ...
                                                           IsPMCVariance = (p.IsPMCVariance == null) ? false : Convert.ToBoolean(p.IsPMCVariance)
    ...
    public static void AddUpdate(DisplayFollowupIssue _FollowupIssue)
    {
            ActionLogger.Logger.WriteFollowUpErrorLog("Issue AddUpdate request: " );
            ...
            try
            {
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    ...
                    if (_followUpIssueLst == null)
                    {
                        ...
                        _followUpIssueLst.IssueId = _FollowupIssue.IssueId == null ? Guid.NewGuid() : _FollowupIssue.IssueId;

                        _followUpIssueLst.IsPMCVariance = _FollowupIssue.IsPMCVariance; // Need to check this code before uncommenting, doesn't work on srrver
                    ...
                    DataModel.SaveChanges();
                    ActionLogger.Logger.WriteFollowUpErrorLog("Issue AddUpdate request success: ");

                }
            }
            catch(Exception ex)
            {
                ActionLogger.Logger.WriteFollowUpErrorLog("Issue AddUpdate exception: " + ex.Message);
            }
    ...
    public static void Delete(Guid IssueID)
    {
        using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
        {
            ...
            {
                if (_followUpObj.IsResolvedFromCommDashboard != true)
                {
