...
namespace MyAgencyVault.BusinessLibrary
{
    ...
    public class PolicyPaymentEntriesPost
    {
        ...
        public static double GetAllPaymentEntriesSumInRange(DateTime FromDate, DateTime ToDate, Guid PolicyID)
        {
            ...
            try
            {
                var list = GetPolicyPaymentEntryPolicyIDWise(PolicyID).Where(p => p.InvoiceDate >= FromDate).Where(p => p.InvoiceDate <= ToDate).ToList<PolicyPaymentEntriesPost>();
                if (list != null && list.Count > 0)
                {
                    sum = (double)list.Sum(x => x.PaymentRecived);
                }
                ActionLogger.Logger.WriteLog("GetAllPaymentEntriesSumInRange request: Sum found as -" + sum, true);
            }
            catch (Exception ex)
            {
